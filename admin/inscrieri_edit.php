...
$result = queryMysql ("SELECT * FROM inscrieri where id=$id");
$row = mysqli_fetch_array($result);
include_once '../include/db_rows_inscrieri.php';
echo "<h1 style='text-align:center;margin-top:20px;'>Detalii solicitant $nume $prenume</h1>";
?>
<body>
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">   
        ...
        <label>Evenimentul solicitat</label>
        <br>
        <input disabled type="text" name="evenimentID" value="<?php echo $evenimentID; ?>">
        <hr>    

        <label>Mesaj voluntar</label>
        <br>
        <textarea name="mesaj" value="<?php echo $mesaj; ?>"><?php echo $mesaj; ?></textarea>
        <hr>
